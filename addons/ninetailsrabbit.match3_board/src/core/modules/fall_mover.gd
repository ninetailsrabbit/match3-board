...
func calculate_fall_movements_on_column(column: int) -> Array:
	...
	while pieces_can_fall_in_column(column):
		...
		var from_cells: Array[Match3GridCellUI] = []
		var to_cells: Array[Match3GridCellUI] = []
		...
		for cell: Match3GridCellUI in column_cells:
			if cell.has_piece():
				if cell.piece.can_be_moved and not cell.is_bottom_border() and cell.neighbour_bottom.is_empty():
					...
					var to_cell: Match3GridCellUI = cell.neighbour_bottom
				...
				if board.configuration.fill_mode_is_side() and cell.neighbour_bottom and cell.neighbour_bottom.has_piece():
		...
		if from_cells.size() > 0 and to_cells.size() > 0:
			for from_cell_index in from_cells.size():
				if from_cell_index < to_cells.size():
					var from_cell: Match3GridCellUI = from_cells[from_cell_index]
					var to_cell: Match3GridCellUI = to_cells[from_cell_index]
